    ...
    public class BuildAssets : EditorWindow
    {
        ...
        private static void Build(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, HashSet<string> assetLoadPath, HashSet<string> alwaysIncludedShaders)
        {
            ...
            if (assetsLoadSetting.EncryptAssets)
            {
                ...
                //复制Log信息
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "FileLogs.bytes"), Path.Combine(encryptAssetPath, "FileLogs.bytes"));
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "DependLogs.bytes"), Path.Combine(encryptAssetPath, "DependLogs.bytes"));
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "GroupLogs.bytes"), Path.Combine(encryptAssetPath, "GroupLogs.bytes"));
            }
            //存储记录所有资源的加载路径
        ...
        private static void SaveLoadLog(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, Dictionary<string, LoadFile> loadFiles, Dictionary<string, LoadDepend> loadDepends, Dictionary<string, LoadGroup> loadGroups)
        {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "FileLogs.bytes")))
            {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "DependLogs.bytes")))
            {
                StringBuilder sb = new StringBuilder();
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, "GroupLogs.bytes")))
            {
                StringBuilder sb = new StringBuilder();
        ...
        private static void SaveBundleVersionFile(string bundlePackagePath, AssetBundleManifest manifest, AssetsLoadSetting assetsLoadSetting, bool encrypt)
        {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(bundlePackagePath, "VersionLogs.bytes")))
            {
                StringBuilder sb = new StringBuilder();
