...
namespace BM
{
    public static partial class AssetComponent
    {
        ...
        {
            //string path = "https://gitee.com/mztn/game001_battle_hero/blob/master/BundleData/AllBundle/VersionLogs.bytes";
            byte[] data = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, bundlePackageName, "VersionLogs.bytes"));
            //byte[] data = await DownloadBundleHelper.DownloadDataByUrl(path);
            if (data == null)
            {
